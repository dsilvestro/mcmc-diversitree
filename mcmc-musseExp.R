...
cat(paste("Writing to File: ", opt$args[1], "_", opt$args[3], "_mcmcEXP.log\n", sep=""))
	...
	for (it in 1:iterations){
	  ...
	  # Appending % of accepted states
		if ( (it %% sampling_freq == 0) & (it >= burnin) ) {
			cat(sprintf("%s\t", c(real_it, accepted_lik, accepted_prior, acc/it, J, accepted_state, current_exp_rate_hp)), "\n", append=TRUE, file=paste(opt$args[1], "_", opt$args[3], "_mcmcEXP.log", sep=""))
